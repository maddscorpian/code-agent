...
**Split LLM instances (`agent/agent_core.py`):** Two separate `Ollama()` instances are created — `planner_llm` (`num_ctx=4096, temperature=0.1`) and `synth_llm` (`num_ctx=8192, num_predict=4096, temperature=0.15, top_p=0.95`). The planner prompt is only ~1K–2K tokens; giving it 4K context keeps it fast. The synthesizer handles gathered tool output (typically 3K–7K tokens) and needs 8K. `num_predict=4096` ensures the synthesizer can output long answers (full Feign client tables, all endpoints). Using 16384 for both caused the KV cache to be 4× too large, forcing CPU offloading and 20–60 s wait times. `AgentLoop.__init__` accepts `planner_llm=None` (falls back to `llm` if not provided). `RAGChain` (fallback path) uses `num_ctx=8192`.
...
**Endpoint node DTOs in graph (`graph/graph_builder.py`):** `_add_spring_nodes()` copies `request_dto` and `response_dto` from the endpoint digest into the graph node. Without this, `describe_feature()` and `get_all_endpoints` always show empty DTOs even when the parser extracted them correctly.

**`get_all_endpoints` includes DTOs (`agent/tools.py`):** The tool output now includes `REQUEST_DTO` and `RESPONSE_DTO` columns from the digest so the LLM can report real DTO class names instead of hallucinating "not specified".
